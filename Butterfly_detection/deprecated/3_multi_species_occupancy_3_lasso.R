...
qr(cbind(rep(1, nrow(x_det_traits)), x_det_traits)) 
...
MSOM4_params = jags_params = c("mu_alpha_null_l1", "alpha_null_l1", "alpha_coef_l1", "alpha_null_l2", "alpha_coef_l2", "beta_null", "beta_coef")

MSOM4_samples = jags(data = MSOM4_data, 
                     inits = MSOM4_inits, 
                     parameters.to.save = MSOM4_params, 
                     model.file = "Butterfly_detection/jags_models/MSOM_4_lasso.txt",
                     n.chains = n_chains, n.adapt = n_adapt, n.burnin = n_burnin, n.iter = n_iter, n.thin = n_thin, 
                     parallel = T, n.cores = n_cores, DIC = T)
saveRDS(MSOM4_samples, file = "Data/models_fit/MSOM/MSOM_4_lasso.RDS")